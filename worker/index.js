...
function buildSummary(records, source) {
  ...
  for (const record of complete) {
    ...
    const deckKey = `${deck}\u0000${ownId ?? "?"}`;
    const matchupKey = `${deckKey}\u0000${opponentId ?? "?"}`;
    ...
    if (!opponents.has(opponentKey)) opponents.set(opponentKey, outcomeBucket(opponentKey, opponent));
    addOutcome(opponents.get(opponentKey), record, result, turn);
    opponents.get(opponentKey).classId = opponentId;
    ...
    matchups.get(matchupKey).opponent = opponent;
  ...
  const recent = complete
    .slice()
    .sort((a, b) => new Date(b.end || b.at || 0).getTime() - new Date(a.end || a.at || 0).getTime())
    .slice(0, 30)
    .map((record) => ({
      id: record.id,
      at: record.at || null,
  ...
  return {
    ...
    decks: sortedBuckets(decks),
    opponentClasses: sortedBuckets(opponents),
    matchups: sortedBuckets(matchups),
