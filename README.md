...
Worker 会按对局内容生成稳定指纹，上传相同对局时复用同一个 R2 对象；读取旧数据时也会按指纹去重，因此历史上已经存在的重复文件不会重复计入统计。Tracker 上传队列和 Worker 都只接受已完成的轮换模式对局，明确标记为无限模式或未完成的记录会被忽略。首页有效对局卡片下方会显示扫描总数、去重数量和排除的无限模式数量；旧 R2 文件不会自动删除，如需清理存储应先备份后再手动处理。
