...
Worker 会按对局内容生成稳定指纹，上传相同对局时复用同一个 R2 对象；读取旧数据时也会按指纹去重，因此历史上已经存在的重复文件不会重复计入统计。上传入口和 Worker 都会过滤已明确标记为无限模式的对局。首页有效对局卡片下方会显示扫描总数、去重数量和排除的无限模式数量；旧 R2 文件不会自动删除，如需清理存储应先备份后再手动处理。
